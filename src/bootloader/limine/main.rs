...
#[unsafe(link_section = ".limine_reqs")]
#[used]
static STACK_SIZE: StackSizeRequest =
    StackSizeRequest::new().with_size((size_of::<Page>() * 512) as u64);
...
impl kernel::BootParms for BootParms {
            ...
            fn access_phy_page(
                ...
                Guard {
                    ...
                    ptr: ptr::with_exposed_provenance_mut::<Page>(
                        self.rf.hhdm_offset + usize::from(phy_page_number) * size_of::<Page>(),
                    ),
                }
            }
        }
    ...
    fn kernel_address(&self) -> kernel::KernelAddress {
        ...
        let pbase = self.kernel_pbase.exact_div(size_of::<Page>());
        let vbase = self.kernel_vbase.exact_div(size_of::<Page>());

        let text_offset = 0;
        let text_length = (addr_of!(KERNEL_TEXT_RO_BOUNDARY).addr() - self.kernel_vbase)
            .exact_div(size_of::<Page>());
